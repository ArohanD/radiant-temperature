...
def serve(web_dir: Path | None = None, port: int = 0) -> str:
    """Start a daemon HTTP server in `web_dir` and return its URL."""
    import http.server
    import socketserver
    import threading

    serve_dir = str((web_dir or WEB).resolve())
    ...
    t.start()
    return f"http://localhost:{bound_port}/index.html"


_SCREENSHOT_VARIANTS = {
    "baseline_tmrt": {"rasters": {"tmrt_h15"}, "show_planted_points": False, "show_disks": None},
    "shadow_15h":    {"rasters": {"shadow_h15"}, "show_planted_points": False, "show_disks": None},
    "dtmrt_mature":  {"rasters": {"dtmrt_mature"}, "show_planted_points": True, "show_disks": "mature"},
}


def _patch_index_for_shot(src_html: str, cfg: dict) -> str:
    import re as _re
    html = src_html
    html = html.replace("<head>", "<head>\n<style>#ui, #info, #hover {display:none !important;}</style>", 1)
    target = cfg["rasters"]
    m = _re.search(r'const\s+MANIFEST\s*=\s*(\{.*?\});\s*\n', html, flags=_re.DOTALL)
    if not m:
        raise RuntimeError("MANIFEST literal not found in index.html")
...
def capture_screenshots(aoi: str | None = None, prefix: str | None = None,
    ...
    out_dir = output_dir or (REPO / f"figures/{AOI_NAME}/slides")
    out_dir.mkdir(parents=True, exist_ok=True)

    port = 8767
    server = _sp.Popen([_sys.executable, "-m", "http.server", str(port)],
                       cwd=str(WEB), stdout=_sp.DEVNULL, stderr=_sp.DEVNULL)
    written = {}
    try:
        for _ in range(10):
            try:
                _urlreq.urlopen(f"http://localhost:{port}/index.html", timeout=1).read()
                break
            except Exception:
                _time.sleep(0.5)
        for tag, cfg in _SCREENSHOT_VARIANTS.items():
            patched = _patch_index_for_shot(src_html, cfg)
            variant_path = WEB / f"_shot_{tag}.html"
            variant_path.write_text(patched)
            url = f"http://localhost:{port}/_shot_{tag}.html"
            out_png = out_dir / f"webapp_{tag}.png"
            _sp.run([
